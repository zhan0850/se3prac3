...
int parse_input(char *line)
{
  ...
  if (!strcasecmp(line,"a")) return GO_LEFT; // left
  if (!strcasecmp(line,"d")) return GO_RIGHT; // right
  if (!strcasecmp(line,"w")) return GO_UP; // up
  if (!strcasecmp(line,"s")) return GO_DOWN; // down
